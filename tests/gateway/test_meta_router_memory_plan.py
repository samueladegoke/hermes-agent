...
from gateway.meta_router import _MODE_RULES, _RULES
...
def test_active_override_zero_hit_fails_closed_instead_of_code_half():
    overrides = {
        "keyword_weight_adjustments": {"code": {"weight_multiplier": 10.0}},
        "type_priority": ["code", "audit", "research", "production", "integration", "config", "design"],
        "confidence_thresholds": {"min_confidence_to_route": 0.0},
    }

    task_type, mode, confidence = runtime._classify_with_overrides(
        "blorple zanzibart quuxleton",
        overrides,
        _RULES,
        _MODE_RULES,
    )

    assert (task_type, mode, confidence) == ("code", "execute", 0.0)


def test_active_override_low_confidence_threshold_fails_closed_instead_of_code_half():
    overrides = {
        "keyword_weight_adjustments": {},
        "type_priority": ["research", "code", "audit", "production", "integration", "config", "design"],
        "confidence_thresholds": {"min_confidence_to_route": 0.9},
    }

    task_type, mode, confidence = runtime._classify_with_overrides(
        "Fix and research; check result.",
        overrides,
        _RULES,
        _MODE_RULES,
    )

    assert (task_type, mode, confidence) == ("research", "review", 0.0)
