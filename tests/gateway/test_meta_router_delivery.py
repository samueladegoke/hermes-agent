...
def test_format_routed_response_delivery_gate_rejection_does_not_claim_threshold_miss():
    result = Phase2Result(
        request_id="rid-4",
        task_type="research",
        state_dir=Path("/tmp/state-4"),
        routing_artifact_version="candidate-0054",
        passed=False,
        score=68.0,
        verdict="ACCEPTABLE",
        threshold=65,
        ...
        score_card=None,
        ref_entry=None,
        delivery_path="/tmp/state-4/delivery.json",
        fix_prompt_path=None,
        error=None,
        notes=["som_status=complete"],
    )
    ...
    text = format_routed_response(
        "Browser-harness update answer",
        result,
        directive="[META-ROUTER | research | execute]",
    )

    assert "missed the final score threshold" not in text
    assert "downstream delivery gate rejected" in text
    assert "Score gate: FAIL (delivery gate rejected; score 68.0 met threshold 65.0)" in text
    assert "Oracle: PASS" in text
    assert "ADV_PASS: PASS (0 findings)" in text
