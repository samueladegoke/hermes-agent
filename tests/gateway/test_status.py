...
class TestGatewayPidState:
    ...
    def test_get_running_pid_removes_stale_foreign_pid_record(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        pid_path = tmp_path / "gateway.pid"
        pid_path.write_text(json.dumps({
            "pid": 99999,
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_records_platform_details(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))

        status.write_runtime_status(
            platform="telegram",
            platform_state="retrying",
            error_code="telegram_start_polling_error",
            error_message="startup failed during start_polling",
            platform_details={
                "startup_stage": "start_polling",
        ...
        payload = status.read_runtime_status()
        assert payload["platforms"]["telegram"]["details"] == {
            "startup_stage": "start_polling",
            "transport_mode": "polling",
            "health": "degraded",
            "retry_count": 2,
