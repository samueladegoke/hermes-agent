...
class TestSummaryTargetRatio:
    ...
    def test_default_threshold_is_85_percent(self):
        """Default compression threshold should be 85% of context."""
        with patch("agent.context_compressor.get_model_context_length", return_value=100_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.85
        assert c.threshold_tokens == 85_000

    def test_threshold_floor_still_applies_at_minimum_context(self):
        """Small contexts still respect the 64K floor when 85% would be lower."""
        with patch("agent.context_compressor.get_model_context_length", return_value=64_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_tokens == 64_000

    def test_threshold_exact_boundary_triggers_compression(self):
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_tokens == 170_000
        assert c.should_compress(170_000) is True
        assert c.should_compress(169_999) is False
